...
$arrCodeProp = ['ID', 'DEPTH_LEVEL', '~NAME', '~SECTION_PAGE_URL'];
...
if($this->StartResultCache())
{
	...
	else
	{
		...
		while($arSection = $rsSections->GetNext())
		{
			...
			if (isset($arSection["UF_SECTION_ICON_IN_MENU"])) {
				$imgId = is_numeric($arSection["UF_SECTION_ICON_IN_MENU"]) ? intval($arSection["UF_SECTION_ICON_IN_MENU"]) : 0;
				if ($imgId && ($imgUrl = CFile::GetPath($imgId))) {
					$arRes['IMG_ID'] = $imgId;
					$arRes['IMG_SRC'] = $imgUrl;
				}
			}
...
foreach($arResult["SECTIONS"] as $arSection)
{
	...
	foreach ($arSection as $key => $value) {
		if (!in_array($key, $arrCodeProp)) {
			$arrLinks[3][$key] = $value;
		}
	}

	$aMenuLinksNew[$menuIndex++] = $arrLinks;
}
